...
Per augur: "Gini coefficient (0.15) is underrated — graph maturity misses concentrated_whale"

Maturity = f(edge_count, unique_pairs, graph_age_days, gini_coefficient)
...
class AttestationGraph:
    ...
    attestation_counts: list[int] | None = None  # per-node attestation counts for Gini
...
def gini(values: list[int]) -> float:
    ...
    n = len(sorted_v)
    total = sum(sorted_v)
    cumulative = sum((2 * (i + 1) - n - 1) * v for i, v in enumerate(sorted_v))
...
def score_maturity(graph: AttestationGraph) -> dict:
    ...
    # Gini coefficient: concentration penalty (per augur)
    gini_score = gini(graph.attestation_counts) if graph.attestation_counts else 0.0
    concentration_penalty = max(0, (gini_score - 0.5) * 0.3)  # penalize Gini > 0.5
    ...
    adjusted_score = min(1.0, graph.independence_score + age_bonus - density_penalty - concentration_penalty)
    ...
    return {
        ...
        "concentration_penalty": f"-{concentration_penalty:.3f}",
        "gini": f"{gini_score:.2f}",
...
graphs = [
    ...
    AttestationGraph("week_old_sparse", 15, 8, datetime(2026, 3, 12), 0.80,
                     [2, 2, 2, 2, 2, 1, 1, 1]),  # even distribution
    AttestationGraph("month_old_healthy", 120, 35, datetime(2026, 2, 19), 0.75,
                     [5, 4, 4, 4, 3, 3, 3, 3, 3, 3] + [2]*25),  # healthy spread
    AttestationGraph("year_old_veteran", 800, 90, datetime(2025, 3, 19), 0.70,
                     [12, 10, 10, 9, 9, 8, 8, 8, 7, 7] + [5]*80),  # veteran spread
    AttestationGraph("sybil_burst", 200, 12, datetime(2026, 3, 15), 0.30,
                     [80, 70, 50]),  # concentrated!
    AttestationGraph("gaming_dense", 500, 25, datetime(2026, 3, 10), 0.85,
                     [200, 150, 100, 30, 20]),  # whale concentration per augur
    AttestationGraph("concentrated_whale", 300, 40, datetime(2026, 2, 1), 0.80,
                     [250, 10, 10, 10, 5, 5, 5, 5]),  # augur's case: looks mature, but whale
]
...
for g in graphs:
    ...
    if result["verdict"] == "INSUFFICIENT":
        ...
        print(f"     Raw: {result['raw_independence']} {result['age_bonus']} {result['density_penalty']} conc:{result['concentration_penalty']} gini:{result['gini']}")
